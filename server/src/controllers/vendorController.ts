...
export const vendorLogin = CatchErrorFunc(
...
export const getVendorProfile = CatchErrorFunc(
  async (req: Request, res: Response) => {
    const user = req.user;
    console.log(user);
    if (user) {
...
export const updateVVendorProfile = CatchErrorFunc(
  async (req: Request, res: Response) => {
...
export const updateVendorProfilePhoto = CatchErrorFunc(
  async (req: Request, res: Response) => {
    const user = req.user;
    if (user) {
      const vendor = await VendorModel.findById(user._id);
...
export const updateVeendorServvice = CatchErrorFunc(
  async (req: Request, res: Response) => {
    const user = req.user;
    if (user) {
      ...
      throw new HandleError(
        "unable to change user service status because there are no foods available on the system",
        404
      );
    } else {
      throw new HandleError("vendor details not found", 404);
...
export const createFood = CatchErrorFunc(
  async (req: Request, res: Response) => {
    const user = req.user;
    ...
    if (user) {
      const vendor = await VendorModel.findById(user._id);
      console.log(vendor);
      const { name, foodType, price, readyTime, category, description } = <
        CreateFoodInputs
      >req.body;
      const files = req.files as [Express.Multer.File];
      const images = files.map((file: Express.Multer.File) => file.filename);
